...
try:
    ...
    from cvtxtclient.models.motor import Motor as TXTApiMotor, Direction as TxtApiDirection
...
class TxtApiConverter:
    ...
    def to_api(self, device: Device) -> Any:
        ...
        if isinstance(device, CounterMotor):
            ...
            if device.speed < 0:
                new_motor.direction = TxtApiDirection.CCW
            else:
                new_motor.direction = TxtApiDirection.CW
            # Create a counter
            ...
            # Set the direction of the motor based on the speed.
            if device.speed < 0:
                new_device.direction = TxtApiDirection.CCW
            else:
                new_device.direction = TxtApiDirection.CW
    ...
    def from_api(self, device: Any, max_id: Optional[int] = None) -> Device:
        ...
        if isinstance(device, TXTApiMotor):
            ...
            if device.direction == TxtApiDirection.CCW and speed > 0:
                speed = -speed
            ...
            # If the direction is CCW, set the speed to negative.
            if device_motor.direction == TxtApiDirection.CCW and speed > 0:
                speed = -speed
            mapped_device.speed = speed
